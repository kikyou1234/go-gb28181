...
func SipRecordList(to *Channels, start, end int64) (*Records, error) {
	...
	if !ok {
		Logger.Error("设备不在线", zap.Any("channel id", to.DeviceID))
	...
	go func() {
		...
		if response.StatusCode() != http.StatusOK {
			Logger.Error("sip message record info error")
		}
	}()
...
func sipMessageRecordInfo(u Devices, body []byte) error {
	...
	if err := utils.XMLDecode(body, message); err != nil {
		Logger.Error("Message Unmarshal xml err:", zap.Any("body :", string(body)), zap.Error(err))
		return err
	}
	Logger.Info("MessageRecordInfoResponse", zap.Any("deviceId", u.DeviceID), zap.Any("msg num", message.SumNum), zap.Any("message", len(message.Item)))
	...
	if list, ok := _recordList.Load(recordKey); ok {
		...
		if info.num == message.SumNum || len(message.Item) < 10 {
			// 获取到完整数据
