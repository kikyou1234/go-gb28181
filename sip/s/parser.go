...
func (p *parser) start() {
	...
	for !p.isStop {
		...
		if err != nil {
			Logger.Error("parserMessage getStartLine faileed", zap.Error(err))
			continue
		}
		if isRequest(startLine) {
			...
			Logger.Error("undefind startlint type", zap.Any("start line", startLine))
			continue
		}
		if termErr != nil {
			Logger.Error("termErr", zap.Error(err))
			continue
		}
		...
		flushBuffer := func() {
			if buffer.Len() > 0 {
				...
				if err == nil {
					...
					Logger.Warn("skip header'due to error: ", zap.Any("error", err))
				}
				buffer.Reset()
			}
		}
